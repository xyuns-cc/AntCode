...
class BatchSender:
    ...
    async def write(self, entry: LogEntry) -> bool:
        ...
        if self._backpressure_state == BackpressureState.CRITICAL and self._config.drop_on_critical:
            self._total_dropped += 1
            return False
    ...
    async def _update_backpressure_state(self) -> None:
        ...
        if new_state != self._backpressure_state:
            ...
            logger.debug(f"[{self.run_id}] Backpressure 状态变更: {old_state.value} -> {new_state.value}")
