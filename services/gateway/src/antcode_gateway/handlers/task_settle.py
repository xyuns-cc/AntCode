...
def _resolve_legacy_settle_until_ts() -> int:
...
def _legacy_settle_argv(worker_id: str) -> str:
    ...
    """
    until_ts = _resolve_legacy_settle_until_ts()
    if until_ts <= 0:
        # 未配置或 0:保持旧行为(永远接受 legacy),兼容当前部署
        return worker_id
    now = int(time.time())
    if now >= until_ts:
        return ""  # 已过滚动升级窗口,关闭 legacy 通道
    return worker_id
