...
2. 已分配 run 必须先成功发出 control 取消指令，才允许写 CANCELLED。
3. runtime CAS 结果必须如实上报 —— 终态冲突时报失败，不谎报取消成功。
"""
...
async def cancel_latest_task_run(task_id: str, user_id: int) -> bool:
    ...
    if execution is None:
        return False
    # P1-FN-01: 未分配 run 先走 CAS 取消(见模块注释第 1 条)。
    if is_unassigned_task_run(execution):
        if await stop_unassigned_task_run(execution, user_id):
            return True
        execution = await TaskRun.get_or_none(run_id=execution.run_id)
        if execution is None:
            return False
    if execution.worker_id and not await try_send_task_cancel(execution, user_id):
        return False
    # P1-FN-01: runtime CAS 结果必须逐 run 如实上报 —— 终态冲突时返回失败
    # 而不是谎报取消成功。
