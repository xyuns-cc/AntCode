...
async def get_configs_by_category(current_admin=Depends(get_current_super_admin)):
    """按分类获取系统配置（仅超级管理员可访问）"""
...
async def get_config(config_key: str, current_admin=Depends(get_current_super_admin)):
    """获取单个系统配置（仅超级管理员可访问）"""
...
async def delete_config(config_key: str, current_admin=Depends(get_current_super_admin)):
    """删除系统配置（仅超级管理员可访问）"""
...
async def reload_configs(current_admin=Depends(get_current_super_admin)):
...
async def initialize_default_configs(current_admin=Depends(get_current_super_admin)):
    """初始化默认配置（仅超级管理员可访问）"""
