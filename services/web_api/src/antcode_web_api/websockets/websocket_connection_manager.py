...
class HeartbeatManager:
    ...
    async def start(self, connection_pool, on_timeout):
        """启动心跳检测"""
        if self._running:
            return

        self._running = True
        self._task = asyncio.create_task(self._heartbeat_loop(connection_pool, on_timeout))
    ...
    async def _heartbeat_loop(self, pool, on_timeout):
        """心跳循环"""
        while self._running:
            try:
    ...
    async def _send_pings(self, pool, on_timeout):
        """发送心跳"""
...
class WebSocketConnectionManager:
    ...
    async def start(self):
        ...
        # 启动心跳管理器
        await self.heartbeat_manager.start(self.connection_pool, self._handle_heartbeat_timeout)
