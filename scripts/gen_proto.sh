...
fix_imports() {
    ...
    if [ -f "$file" ]; then
        ...
        # T6-T3: 补齐 data_pb2 / control_pb2 的 import fix（原脚本只覆盖
        # common_pb2 + 已弃用的 gateway_pb2，导致 grpc 桩文件里
        # `import data_pb2` 无法被 antcode_contracts 包解析）
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS
            sed -i '' 's/^import common_pb2 as/from . import common_pb2 as/g' "$file"
            sed -i '' 's/^import control_pb2 as/from . import control_pb2 as/g' "$file"
            sed -i '' 's/^import data_pb2 as/from . import data_pb2 as/g' "$file"
        else
            # Linux
            sed -i 's/^import common_pb2 as/from . import common_pb2 as/g' "$file"
            sed -i 's/^import control_pb2 as/from . import control_pb2 as/g' "$file"
...
for _mod in ('control_pb2', 'data_pb2', 'control_pb2_grpc', 'data_pb2_grpc'):
    try:
        __import__(f'antcode_contracts.{_mod}')
        logger.info('  OK  %s 导入成功', _mod)
    except ImportError as e:
        logger.error('  FAIL %s 导入失败: %s', _mod, e)
        sys.exit(1)
