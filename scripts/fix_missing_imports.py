...
def fix_missing_imports(file_path: Path):
    ...
    content = re.sub(
        r"from antcode_core\.common\.serialization import", "from src.common.serialization import", content
    )

    # 修复 response 导入 - 暂时保持使用 src.core
    content = re.sub(r"from antcode_core\.common\.response import", "from src.core.response import", content)

    # 修复 resilience 导入
    content = re.sub(r"from antcode_core\.infrastructure\.resilience", "from src.core.resilience", content)

    # 修复 cache 导入
    content = re.sub(r"from antcode_core\.infrastructure\.cache", "from src.infrastructure.cache", content)
