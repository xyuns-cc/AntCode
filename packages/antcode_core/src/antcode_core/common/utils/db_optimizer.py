...
class DatabaseOptimizer:
    ...
    @staticmethod
    async def bulk_get_or_create(model_class, items, unique_fields, batch_size=100):
        """
        批量获取或创建对象
            ...
            if updated_objects:
                try:
                    await model_class.bulk_update(
                        updated_objects,
                        fields=[field for field in updates[0] if field != key_field],
                    )
    ...
    @staticmethod
    async def optimized_paginate(queryset, page, size, use_cursor=False, cursor_field="id"):
        """
        优化的分页查询
    ...
    @staticmethod
    async def batch_delete(model_class, filters, batch_size=1000):
        """
        批量删除对象
...
def _serialize_result(result):
    ...
    if isinstance(result, (list, tuple)):
        return {
            ...
            "__data__": [_serialize_result(i) for i in result],
        }
...
def cached_query(ttl=300, namespace=None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            from antcode_core.infrastructure.cache import query_cache
