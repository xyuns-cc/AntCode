...
VALID_STATE_TRANSITIONS: dict[str, list[str]] = {
    BatchStatus.PENDING.value: [BatchStatus.RUNNING.value, BatchStatus.CANCELLED.value],
    ...
    BatchStatus.PAUSED.value: [BatchStatus.RUNNING.value, BatchStatus.CANCELLED.value],
    BatchStatus.COMPLETED.value: [],  # 终态，不能转换
    BatchStatus.FAILED.value: [],  # 终态，不能转换
    BatchStatus.CANCELLED.value: [],  # 终态，不能转换
}
...
class CrawlBatchService(BaseService):
    ...
    async def _publish_batch_event(
        ...
        """Persist a crawl lifecycle event in the PostgreSQL outbox."""
        await scheduler_outbox_service.enqueue(
            event_type=event,
            aggregate_type="crawl_batch",
            aggregate_id=batch_id,
            payload={"batch_id": batch_id},
            connection=connection,
        )

    async def _save_batch_with_event(self, batch: CrawlBatch, event: str) -> None:
        async with in_transaction("default") as conn:
            await batch.save(using_db=conn)
            await self._publish_batch_event(event, batch.public_id, connection=conn)
    ...
    async def start_batch(self, batch_id: str) -> CrawlBatch:
        ...
        await self._save_batch_with_event(batch, "batch_started")

        logger.info(f"启动批次: batch_id={batch_id}, seeds={len(batch.seed_urls)}")
    ...
    async def pause_batch(self, batch_id: str) -> CrawlBatch:
        ...
        await self._save_batch_with_event(batch, "batch_paused")
    ...
    async def resume_batch(self, batch_id: str) -> CrawlBatch:
        ...
        # 更新批次状态
        batch.status = BatchStatus.RUNNING
        await self._save_batch_with_event(batch, "batch_resumed")
    ...
    async def complete_batch(self, batch_id: str, success: bool = True) -> CrawlBatch:
        ...
        **P1-32 修复**：``batch.save()`` 只带主键做 UPDATE，同一批次并发被 loop
        ...
        if not updated:
            ...
            logger.warning(
                f"complete_batch CAS 失败: batch_id={batch_id} expected=running actual={current} target={target_status}"
            )
    ...
    async def list_batches(
        ...
        # 分页查询
        batches, total = await self.query.paginate(query, page, size, order_by="-created_at")
    ...
    async def cancel_all_running_batches(self, project_id: str | None = None) -> int:
        """取消所有运行中的批次
    ...
    async def cleanup_test_batches(self, project_id: str | None = None, days: int = 7) -> int:
        """清理过期的测试批次
