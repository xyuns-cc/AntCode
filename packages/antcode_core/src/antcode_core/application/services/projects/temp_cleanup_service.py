...
class TempCleanupService:
    ...
    def temp_dir(self):
        """获取临时目录"""
        return file_storage_service.get_file_path("temp")

    async def start_background_cleanup(self, interval_hours=6):
        """启动后台清理"""
        if self._running:
    ...
    async def _cleanup_loop(self, interval_hours):
        """清理循环"""
        while self._running:
            try:
    ...
    async def cleanup_temp_files(self, max_age_hours=None):
        """清理过期临时文件"""
        ...
        try:
            ...
            if cleaned_count > 0:
                logger.info(f"已清理{cleaned_count}个文件 释放{total_size / 1024 / 1024:.2f}MB")
    ...
    async def get_temp_stats(self):
        ...
        try:
            ...
            return {
                ...
                "oldest_file": datetime.fromtimestamp(oldest_file).isoformat()
                if oldest_file
                else None,
                "newest_file": datetime.fromtimestamp(newest_file).isoformat()
                if newest_file
                else None,
            }
