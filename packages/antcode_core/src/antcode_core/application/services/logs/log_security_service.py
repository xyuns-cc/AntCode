...
class LogSecurityService:
    ...
    def _check_rate_limit(self, user_id):
        ...
        # 清理过期记录
        self._access_limits[user_id] = [
            timestamp for timestamp in self._access_limits[user_id] if timestamp > minute_ago
        ]
    ...
    async def get_user_accessible_executions(self, user, limit=100):
        """获取用户可访问的执行记录ID列表"""
        try:
            # 获取用户的任务
...
class EnhancedErrorHandler:
    ...
    def log_error(self, error, context):
        ...
        # 记录详细错误信息
        logger.error(f"错误ID: {error_id}, 类型: {error_type}, 消息: {str(error)}, 上下文: {context}")
