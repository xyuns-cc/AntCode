...
class LogPerformanceMonitor:
    ...
    async def record_metric(
        self, operation, duration, bytes_processed=0, lines_processed=0, success=True, error_message=None
    ):
        """记录性能指标"""
    ...
    def get_recent_metrics(self, minutes=5):
        ...
        recent_metrics = [asdict(metric) for metric in self.metrics_history if metric.timestamp > cutoff_time]
    ...
    def get_slow_operations(self, threshold_seconds=1.0, limit=10):
        """获取慢操作列表"""
        slow_ops = [asdict(metric) for metric in self.metrics_history if metric.duration > threshold_seconds]
...
class LogStatisticsService:
    ...
    def __init__(self):
        ...
        self.user_activity: dict[int, dict[str, Any]] = defaultdict(
            lambda: {"last_seen": None, "requests_today": 0, "total_bytes": 0, "errors": 0}
        )

    def record_user_activity(self, user_id, bytes_processed=0, is_error=False):
        """记录用户活动"""
        ...
        user_stats["last_seen"] = datetime.now(UTC)
    ...
    def get_user_statistics(self):
        ...
        for user_id, stats in self.user_activity.items():
            if stats["last_seen"]:
                ...
                if (datetime.now(UTC) - last_seen).days == 0:
