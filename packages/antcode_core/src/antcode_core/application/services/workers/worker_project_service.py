...
class WorkerProjectService:
    ...
    async def record_project_sync(
        ...
        """记录同步状态"""
        worker_project = await WorkerProject.get_or_none(worker_id=worker_id, project_public_id=project_public_id)
        ...
        if worker_project:
            ...
            logger.info(f"同步记录已更新 [Worker {worker_id}@{project_public_id}] 同步次数:{worker_project.sync_count}")
        else:
    ...
    async def delete_worker_project(self, worker_id, project_public_id):
        ...
        worker_project = await WorkerProject.filter(worker_id=worker_id, project_public_id=project_public_id).first()
    ...
    async def get_sync_statistics(self, worker_id=None):
        ...
        return {
            ...
            "avg_sync_count": (sum(r.sync_count for r in all_records) / len(all_records) if all_records else 0),
        }
