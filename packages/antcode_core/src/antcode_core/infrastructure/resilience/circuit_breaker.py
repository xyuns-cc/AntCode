...
class CircuitOpenError(CircuitBreakerError):
    ...
    def __init__(self, name: str, remaining_seconds: float):
        ...
        super().__init__(f"熔断器 '{name}' 已打开，剩余 {remaining_seconds:.1f} 秒恢复")
...
class CircuitBreaker:
    ...
    def _should_trip(self, exception: Exception) -> bool:
        ...
        if self.config.exception_types:
            return any(isinstance(exception, t) for t in self.config.exception_types)
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        if self._state == CircuitState.OPEN and self._open_time:
            remaining_timeout = max(0, self.config.timeout - (time.time() - self._open_time))
