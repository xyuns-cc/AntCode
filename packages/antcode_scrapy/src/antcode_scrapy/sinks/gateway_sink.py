...
class GatewaySpiderDataSink:
    ...
    async def _flush(self) -> bool:
        ...
        async with self._lock:
            ...
            meta_fields = dict(self._pending_meta)
        ...
        if meta_fields:
            ...
            if "status" in meta_fields:
                meta.status = meta_fields["status"]
            if "items_count" in meta_fields:
                try:
                    meta.items_count = int(meta_fields["items_count"])
                except (TypeError, ValueError):
                    pass
            if "last_item_at" in meta_fields:
                meta.last_item_at = meta_fields["last_item_at"]
        ...
        try:
            ...
            if failed:
                logger.warning(
                    f"gateway StreamSpiderData 部分失败: accepted={ack.accepted} "
                    f"failed={failed}"
                )
                return False
            return True
        except Exception as exc:
            logger.error(f"gateway StreamSpiderData 失败: {exc}")
            return False
