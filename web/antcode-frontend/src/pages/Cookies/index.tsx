...
const Cookies: React.FC = () => {
    ...
    // 自动刷新计时器 (Round6 P0-02 lint fix: handleAutoRefresh 依赖用 ref 化避免闭包过期)
    const handleAutoRefreshRef = useRef<() => Promise<void>>(() => Promise.resolve())
    useEffect(() => {
        const timer = setInterval(() => {
            setTimeLeft((prev) => {
                if (prev <= 1) {
                    handleAutoRefreshRef.current()
    ...
    // 每次渲染同步最新 handleAutoRefresh 到 ref, 让 interval 里的调用看到最新闭包
    useEffect(() => {
        handleAutoRefreshRef.current = handleAutoRefresh
    })
