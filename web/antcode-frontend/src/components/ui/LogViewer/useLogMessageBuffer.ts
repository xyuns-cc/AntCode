...
const truncateContent = (content: string): string => {
  if (typeof content !== 'string' || content.length <= MAX_MESSAGE_CONTENT_BYTES) return content
  return `${content.slice(0, MAX_MESSAGE_CONTENT_BYTES)}\n… (前端截断, 原长度 ${content.length} 字符)`
}
...
const useSequenceDeduper = (refs: BufferRefs, maxLines: number) => useCallback((message: LogMessage) => {
  ...
  const key = `${message.type}:${message.sequence}`
...
const useMessageQueue = (refs: BufferRefs, maxLines: number) => {
  ...
  const flush = useCallback(() => {
    ...
    setMessages((previous) => [...previous, ...pending].slice(-maxLines))
  }, [maxLines, refs])
  ...
  const trimPendingToWindow = useCallback(() => {
    ...
    const excess = refs.pendingMessages.current.length - limit
    if (excess <= 0) return
    refs.pendingMessages.current.splice(0, excess)
