...
class CrawlService extends BaseService {
  ...
  /**
   * 通过 axios 走 Bearer 认证下载导出文件，再本地触发下载。
   *
   * 不能用 `window.open(url)`：浏览器新标签页不会带 axios 拦截器注入的
   * Authorization 头，登录用户导出永远 401。这里用 responseType: 'blob'
   * 走鉴权拿字节流，再合成 <a download> 触发下载。
   */
