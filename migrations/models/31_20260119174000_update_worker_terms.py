"""数据口径统一为 worker（更新历史 action 与 env_location 值）"""
...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    async def update_value(table: str, column: str, old: str, new: str) -> None:
        if await table_exists(table) and await column_exists(table, column):
            statements.append(
                f"UPDATE `{table}` SET `{column}` = '{new}' WHERE `{column}` = '{old}'"
            )

    # 审计日志 action 口径统一
    await update_value("audit_logs", "action", "node_create", "worker_create")
    await update_value("audit_logs", "action", "node_update", "worker_update")
    await update_value("audit_logs", "action", "node_delete", "worker_delete")
    await update_value("audit_logs", "action", "node_resource_update", "worker_resource_update")

    # 项目运行时位置统一
    await update_value("projects", "env_location", "node", "worker")
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    ...
    await update_value("audit_logs", "action", "worker_create", "node_create")
    await update_value("audit_logs", "action", "worker_update", "node_update")
    await update_value("audit_logs", "action", "worker_delete", "node_delete")
    await update_value("audit_logs", "action", "worker_resource_update", "node_resource_update")

    await update_value("projects", "env_location", "worker", "node")
