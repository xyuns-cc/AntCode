...
class HealthChecker:
    ...
    async def _check_grpc(self) -> ComponentHealth:
        """检查 gRPC 服务状态"""
        start = time.time()
        try:
            from src.core.config import settings
            ...
            # 检查 gRPC 服务是否运行
            from src.services.grpc import grpc_server
            
            is_running = grpc_server.is_running if hasattr(grpc_server, 'is_running') else True
            connected_nodes = grpc_server.connected_nodes_count if hasattr(grpc_server, 'connected_nodes_count') else 0
            
            return ComponentHealth(
                name="grpc",
                status=HealthStatus.HEALTHY if is_running else HealthStatus.UNHEALTHY,
                latency_ms=(time.time() - start) * 1000,
                message="gRPC 服务运行中" if is_running else "gRPC 服务未运行",
                details={
                    "enabled": True,
