...
def task_list(total: int, page_num: int, size: int, items: list):
    """构建任务列表响应"""
...
def execution_list(total: int, page_num: int, size: int, items: list):
    """构建执行记录列表响应"""
...
class ProjectResponseBuilder:
    ...
    def build_detail(project):
        ...
        bound_node_id = getattr(project, 'bound_node_id', None)
        bound_node_name = getattr(project, 'bound_node_name', None)
        ...
        return ProjectResponse.model_construct(
            ...
            bound_node_id=str(bound_node_id) if bound_node_id else None,
    ...
    @staticmethod
    def build_list(projects: list) -> list:
        """批量构建项目列表响应"""
...
class TaskResponseBuilder:
    ...
    @staticmethod
    def build_list(tasks: list) -> list:
        """批量构建任务列表响应"""
...
class ExecutionResponseBuilder:
    ...
    def build_detail(execution):
        """构建执行记录详情响应"""
        from src.schemas.scheduler import ExecutionResponse
        return ExecutionResponse.from_orm(execution)

    @staticmethod
    def build_list(executions: list) -> list:
        """批量构建执行记录列表响应"""
        from src.schemas.scheduler import ExecutionResponse
