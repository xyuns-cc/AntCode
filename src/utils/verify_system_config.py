...
async def verify_config():
    ...
    try:
        ...
        logger.info(f"✓ 配置缓存已加载: {cache_size} 个配置项")
        ...
        for config_key, settings_key in config_mapping.items():
            if not hasattr(settings, settings_key):
                ...
                logger.error(f"✗ settings缺少属性: {settings_key}")
            else:
                value = getattr(settings, settings_key)
                cached_value = system_config_service.get_cached_config(config_key)
                if cached_value is not None and value == cached_value:
                    logger.info(f"✓ {settings_key} = {value} (已同步)")
                else:
                    logger.warning(f"⚠ {settings_key} = {value}, 缓存值 = {cached_value} (不同步)")
        ...
        for config in configs:
            try:
                if config.value_type == "int":
                    ...
                    assert config.config_value.lower() in ("true", "false", "1", "0", "yes", "no", "on", "off")
                logger.info(f"✓ {config.config_key}: {config.value_type} = {config.config_value}")
            except Exception as e:
                type_errors.append((config.config_key, str(e)))
                logger.error(f"✗ {config.config_key}: 类型错误 - {e}")
        ...
        for config_key, (min_val, max_val) in range_checks.items():
            ...
            if value is not None:
                if isinstance(value, (int, float)) and (value < min_val or value > max_val):
                    ...
                    logger.error(f"✗ {config_key} = {value} (超出范围 {min_val}-{max_val})")
                else:
                    logger.info(f"✓ {config_key} = {value} (范围正常)")
