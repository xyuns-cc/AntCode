...
class QueuedTask:
    ...
    @classmethod
    def from_json(cls, json_str: str) -> "QueuedTask":
...
class BaseQueueBackend(ABC):
    ...
    def _update_stats(self, operation: str, count: int = 1) -> None:
        """更新统计信息
    ...
    def _log_operation(self, operation: str, task_id: str, **kwargs) -> None:
        """统一的操作日志记录
    ...
    def _log_warning(self, message: str, task_id: Optional[str] = None) -> None:
        """统一的警告日志记录
...
# 全局队列后端实例（单例）
_queue_backend_instance: Optional[TaskQueueBackend] = None


def get_queue_backend_type() -> str:
    """获取配置的队列后端类型
...
def get_queue_backend() -> TaskQueueBackend:
    ...
    if backend_type == "memory" or backend_type == "":
        ...
        if not redis_url:
            raise ValueError(
                "QUEUE_BACKEND=redis 时必须设置 REDIS_URL 环境变量"
            )
        from src.services.scheduler.redis_queue import RedisQueueBackend
        _queue_backend_instance = RedisQueueBackend(redis_url)
        logger.info("使用 Redis 任务队列后端 (RedisQueueBackend)")

    else:
        raise ValueError(
            f"无效的 QUEUE_BACKEND 值: {backend_type}，"
            f"支持的值: memory, redis"
        )
...
async def init_queue_backend() -> TaskQueueBackend:
    """初始化并启动队列后端
