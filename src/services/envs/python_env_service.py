...
class PythonEnvService:
    ...
    async def ensure_installed(self, version: str, created_by: int | None = None) -> Dict[str, str]:
        """确保指定版本已安装并返回安装信息。"""
    ...
    async def register_local(self, python_bin: str, created_by: int | None = None) -> Dict[str, str]:
        """注册本地解释器，读取版本并写入数据库。"""
