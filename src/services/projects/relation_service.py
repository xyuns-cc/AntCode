...
class RelationService:
    ...
    async def get_task_by_id(task_id: int) -> Optional[ScheduledTask]:
        """根据ID获取任务"""
        return await ScheduledTask.get_or_none(id=task_id)

    @staticmethod
    async def get_execution_by_id(execution_id: int) -> Optional[TaskExecution]:
        """根据ID获取执行记录"""
        return await TaskExecution.get_or_none(id=execution_id)
    ...
    @staticmethod
    async def delete_project_cascade(project_id: int) -> dict:
        """级联删除项目及其相关数据"""
        deleted = {
            "tasks": 0,
            "executions": 0,
        ...
        try:
            # 1. 获取任务ID并删除执行记录
            task_ids = await ScheduledTask.filter(project_id=project_id).values_list('id', flat=True)
            if task_ids:
                deleted["executions"] = await TaskExecution.filter(task_id__in=list(task_ids)).delete()

            # 2. 删除任务
            deleted["tasks"] = await ScheduledTask.filter(project_id=project_id).delete()
            ...
            deleted["venv_bindings"] = await ProjectVenvBinding.filter(project_id=project_id).delete()

            # 5. 删除节点项目同步记录
            from src.models import NodeProject, NodeProjectFile
            node_projects = await NodeProject.filter(project_id=project_id).all()
            if node_projects:
