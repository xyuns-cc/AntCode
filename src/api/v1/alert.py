...
from src.core.security.auth import TokenData, get_current_user, get_current_super_admin
from src.core.response import success, BaseResponse
from src.models import SystemConfig, User
from src.utils.serialization import from_json, to_json
from src.schemas.alert import (
    AlertConfigRequest, AlertConfigResponse, AlertChannelConfig,
    AlertRateLimitConfig, AlertRetryConfig, WebhookConfig, EmailConfig,
    AlertHistoryResponse, AlertHistoryItem,
    AlertStatsResponse, AlertTestRequest, AlertTestResponse
)
...
async def get_alert_config(
    ...
    email_config = EmailConfig(**email_config_data) if email_config_data else None

    return success(AlertConfigResponse(
...
async def get_alert_history(
    ...
    return success(AlertHistoryResponse(
        items=[AlertHistoryItem(**h) for h in history],
        total=len(history)
    ))
