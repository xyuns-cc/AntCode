...
class RenderSpider(ABC):
    ...
    def __init__(self, **kwargs):
        ...
        self._client: Optional[RenderClient] = None
        self._running = False
    ...
    async def parse(self, response: RenderResponse) -> AsyncGenerator[Union[Dict, str], None]:
        """
        解析响应
    ...
    async def run(self, client: Optional[RenderClient] = None) -> RenderCrawlResult:
        """
        运行爬虫
    ...
    def re_first(self, text: str, pattern: str, default: str = "", flags: int = 0) -> str:
        """正则匹配（第一个）"""
        match = re.search(pattern, text, flags)
        return match.group(1) if match and match.groups() else (match.group() if match else default)
