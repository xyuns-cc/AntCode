...
class RenderConfig:
    ...
    # 自定义命令行参数 (列表形式，如 ["--disable-web-security", "--allow-running-insecure-content"])
    extra_arguments: List[str] = field(default_factory=list)

    # 自定义偏好设置 (字典形式，如 {"credentials_enable_service": False})
    preferences: Dict[str, Any] = field(default_factory=dict)

    # 扩展插件路径列表
    extensions: List[str] = field(default_factory=list)

    # 实验性 flags (字典形式，如 {"temporary-unexpire-flags-m118": "1"})
...
class BrowserPool:
    ...
    def _find_available_port(self) -> int:
        ...
        for port in range(self.config.port_start, self.config.port_end):
            if port not in self._port_map:
                return port
        raise RuntimeError(f"无可用端口 ({self.config.port_start}-{self.config.port_end})")
    ...
    def _create_options(self, browser_id: str, port: int) -> "ChromiumOptions":
        ...
        if self.config.new_env:
            co.new_env(True)  # 每次启动新环境，关闭后自动清理
        else:
            co.set_user_data_path(user_data_path)  # 持久化用户数据
        ...
        for arg in self.config.extra_arguments:
            if "=" in arg:
                # 带值的参数，如 --user-data-dir=/path
                key, value = arg.split("=", 1)
                co.set_argument(key, value)
            else:
                # 无值参数，如 --disable-web-security
                co.set_argument(arg)
    ...
    async def acquire(self, browser_id: Optional[str] = None) -> BrowserInstance:
        """
        获取浏览器实例
