...
class ScreenshotSpider(RenderSpider):
    ...
    async def parse(self, response: RenderResponse) -> AsyncGenerator[Union[Dict, str], None]:
        """抓取并截图"""
        # 渲染页面并截图
        screenshot_response = await self.render(
            response.url,
            screenshot=True
        )
        ...
        yield {
            ...
            "screenshot_size": len(screenshot_response.screenshot) if screenshot_response.screenshot else 0,
        }
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            print(f"抓取完成: {result.items_count} 条数据")
