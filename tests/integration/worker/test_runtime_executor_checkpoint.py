...
"""
        ...
        try:
            ...
            for i in range(5):
                ...
                task = asyncio.create_task(executor.run(exec_plan, runtime_handle, NoOpLogSink()))
                tasks.append(task)
...
"""
        ...
        try:
            ...
            # 启动任务
            asyncio.create_task(executor.run(exec_plan, runtime_handle, NoOpLogSink()))
...
"""
        ...
        try:
            ...
            # 验证日志捕获
            stdout_logs = [log_entry for log_entry in logs if log_entry.stream == LogStream.STDOUT]
            stderr_logs = [log_entry for log_entry in logs if log_entry.stream == LogStream.STDERR]
            ...
            # 验证内容
            stdout_content = " ".join(log_entry.content for log_entry in stdout_logs)
            assert "This is stdout" in stdout_content

            stderr_content = " ".join(log_entry.content for log_entry in stderr_logs)
