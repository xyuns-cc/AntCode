...
class TestAddPluginWithoutEngineChange:
    ...
    async def test_custom_plugin_builds_plan(self):
        ...
        payload = TaskPayload(
            ...
            project_cwd="/tmp/workspace/project",
        )
...
class TestExecPlanGeneration:
    ...
    async def test_spider_plugin_generates_scrapy_plan(self):
        ...
        payload = TaskPayload(
            ...
            workspace_path="/tmp/workspace",
            project_cwd="/tmp/workspace/spider_project",
            kwargs={
                "framework": "scrapy",
    ...
    async def test_render_plugin_rejects_playwright_engine(self):
        """RenderPlugin 明确拒绝 Playwright 引擎"""
        context = RunContext(
            run_id="test-run-005",
            task_id="task-005",
        ...
        assert plugin is not None
        errors = plugin.validate(payload)
        assert errors == ["不支持的 render engine: playwright"]

        with pytest.raises(PluginError, match="不支持的 render engine: playwright"):
            await self.registry.build_plan(context, payload)
