...
EXECUTION_ID_MIN_LENGTH = 8  # 与 min_size=8 一致,避免 lambda 内出现 magic number

# Strategies for generating test data
execution_id_strategy = st.text(
    alphabet=st.characters(whitelist_categories=("Ll", "Lu", "Nd")),
    min_size=EXECUTION_ID_MIN_LENGTH,
    max_size=32,
).filter(lambda x: len(x) >= EXECUTION_ID_MIN_LENGTH)
...
class TestLogStreamerCompleteness:
    ...
    def test_enabled_streamer_sends_all_logs(self, execution_id: str, log_type: str, content: str):
        """
        **Feature: directory-restructure, Property 6: Log Completeness Rule**
        **Validates: Requirements 4.3, 7.5**
    ...
    def test_disabled_streamer_does_not_send(self, execution_id: str, log_type: str, content: str):
        """
        **Feature: directory-restructure, Property 6: Log Completeness Rule**
        **Validates: Requirements 4.3, 7.5**
    ...
    def test_buffered_streamer_sends_all_buffered_logs(self, execution_id: str, log_lines: list[str]):
        """
        **Feature: directory-restructure, Property 6: Log Completeness Rule**
        **Validates: Requirements 4.3, 7.5**
        ...
        async def run_test():
            ...
            if log_lines:
                ...
                all_content = "\n".join(msg["content"] for msg in sender.messages if msg.get("type") == "log_realtime")
                for line in log_lines:
...
class TestLogArchiverCompleteness:
    ...
    def test_archiver_sends_all_file_content(self, execution_id: str, log_type: str, content: bytes):
        """
        **Feature: directory-restructure, Property 6: Log Completeness Rule**
        **Validates: Requirements 4.3, 7.5**
        ...
        async def run_test():
            ...
            try:
                ...
                # Verify all content was sent
                chunks = [msg for msg in sender.messages if msg.get("type") == "log_chunk"]
                ...
                assert reassembled == content, (
                    f"Content mismatch: expected {len(content)} bytes, got {len(reassembled)} bytes"
                )
    ...
    def test_archiver_chunk_checksums_valid(self, execution_id: str, log_type: str, content: bytes):
        """
        **Feature: directory-restructure, Property 6: Log Completeness Rule**
        **Validates: Requirements 4.3, 7.5**
...
class TestLogSystemIntegration:
    ...
    def test_archiver_status_contains_required_fields(self, execution_id: str, log_type: str):
        """
        **Feature: directory-restructure, Property 6: Log Completeness Rule**
        **Validates: Requirements 4.3, 7.5**
