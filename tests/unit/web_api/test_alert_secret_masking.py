...
def _submitted(webhook_id: str, name: str) -> WebhookConfig:
    ...
    return WebhookConfig(id=webhook_id, name=name, url=alert_config_store.SECRET_MASK, levels=["ERROR"], enabled=True)
...
def test_alert_response_masks_webhook_and_email_secrets():
    ...
    assert webhooks[0]["url"] == alert_config_store.SECRET_MASK
    assert email.smtp_password == alert_config_store.SECRET_MASK


def test_masked_webhook_update_preserves_existing_secret():
    merged = alert_config_store.merge_webhooks([_submitted(FIRST_ID, "ops")], [_stored(FIRST_ID, "ops", FIRST_URL)])
...
def test_renamed_webhook_keeps_its_own_secret():
    ...
    merged = alert_config_store.merge_webhooks(
        [_submitted(FIRST_ID, "ops-renamed")], [_stored(FIRST_ID, "ops", FIRST_URL)]
    )
...
def test_swapped_names_do_not_swap_secrets():
    ...
    merged = alert_config_store.merge_webhooks(
        [_submitted(FIRST_ID, "second"), _submitted(SECOND_ID, "first")], existing
    )
...
def test_duplicate_display_names_keep_separate_secrets():
    ...
    merged = alert_config_store.merge_webhooks([_submitted(FIRST_ID, "ops"), _submitted(SECOND_ID, "ops")], existing)
...
def test_deleting_and_reordering_keeps_each_secret():
    ...
    merged = alert_config_store.merge_webhooks([_submitted(SECOND_ID, "second")], existing)
...
def test_masked_url_without_known_id_is_rejected():
    ...
    with pytest.raises(HTTPException) as exc_info:
        alert_config_store.merge_webhooks(
            [WebhookConfig(name="ops", url=alert_config_store.SECRET_MASK)], [_stored(FIRST_ID, "ops", FIRST_URL)]
        )
