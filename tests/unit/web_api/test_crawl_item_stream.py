...
def _entry(entry_id: str, sequence: int) -> tuple[bytes, dict[bytes, bytes]]:
    return (
        entry_id.encode(),
        {
...
async def test_limit_stops_before_reading_later_runs():
    redis = FakeRedis({"ac:spider:data:run-1:-": [_entry("1-0", 1), _entry("2-0", 2)]})
    ...
    assert redis.calls == [("ac:spider:data:run-1", "-", 2)]
...
async def test_reader_pages_without_collecting_entire_stream(monkeypatch):
    ...
    redis = FakeRedis(
        {
            "ac:spider:data:run-1:-": [_entry("1-0", 1), _entry("2-0", 2)],
            "ac:spider:data:run-1:(2-0": [_entry("3-0", 3)],
        }
    )
    reader = CrawlBatchItemReader(redis, FakeConnection(), RedisKeys("ac"))
    ...
    assert redis.calls == [
        ("ac:spider:data:run-1", "-", 2),
        ("ac:spider:data:run-1", "(2-0", 1),
    ]
