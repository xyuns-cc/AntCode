...
async def test_trigger_handlers_return_deterministic_outbox_run_id(monkeypatch, http_request, handler) -> None:
    ...
    if handler is tasks_execute.execute_task:
        response = await handler("task-public", tasks_execute.TaskExecuteRequest(), user, http_request=http_request)
    else:
        response = await handler("task-public", user, http_request=http_request)
...
@pytest.mark.asyncio
async def test_trigger_handler_keeps_mocked_legacy_boolean_compatible(monkeypatch, http_request) -> None:
    monkeypatch.setattr(tasks_execute, "_acquire_trigger_dedup_lock", AsyncMock(return_value=True))
    monkeypatch.setattr(tasks_execute.scheduler_service, "trigger_task_by_user", AsyncMock(return_value=True))

    response = await tasks_execute.trigger_task("task-public", SimpleNamespace(user_id=7), http_request=http_request)
