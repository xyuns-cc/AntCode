...
def test_crawl_redis_preflight_uses_candidate_image_and_minimum_secret() -> None:
    services = _compose()["services"]
    preflight = services["crawl-redis-preflight"]

    assert preflight["image"] == services["migration"]["image"]
    assert preflight["profiles"] == ["crawl-redis-preflight"]
    ...
    assert preflight["secrets"] == ["redis_url"]
    assert preflight["environment"] == {
        "REDIS_URL_FILE": "/run/secrets/redis_url",
        "REDIS_NAMESPACE": "${REDIS_NAMESPACE:?REDIS_NAMESPACE is required}",
    }
    assert preflight["networks"] == ["antcode-control"]


def test_development_preflight_profile_uses_the_runtime_image_contract() -> None:
    preflight = _compose(DEV_COMPOSE)["services"]["crawl-redis-preflight"]

    assert preflight["build"]["dockerfile"] == "infra/docker/Dockerfile.web_api"
    assert preflight["profiles"] == ["crawl-redis-preflight"]
    assert preflight["read_only"] is True
    assert preflight["command"] == ["python", "-m", "scripts.crawl_redis_preflight"]
    assert "${REDIS_NAMESPACE:?" in preflight["environment"]["REDIS_NAMESPACE"]


def test_crawl_redis_preflight_modules_are_in_runtime_image() -> None:
    dockerfile = _script(WEB_API_DOCKERFILE)

    for module in (
        "crawl_redis_preflight_contract.py",
        "crawl_redis_preflight_execution.py",
        "crawl_redis_preflight_scan.py",
        "crawl_redis_preflight.py",
    ):
...
def test_deploy_script_stops_writers_before_the_read_only_redis_gate() -> None:
    ...
    preflight_run = "run --rm --no-deps crawl-redis-preflight"
    ...
    assert deploy.index('stop --timeout "$STOP_TIMEOUT"') < deploy.index(preflight_run)
    ...
    assert deploy.index(preflight_run) < deploy.rindex("up -d --wait --wait-timeout")
