...
class TestWorkerProjectSyncService:
    ...
    async def test_sync_requires_s3_storage(self, mock_worker):
        ...
        with patch(
            ...
            results, info = await service.sync_projects_to_worker_with_info(mock_worker, ["proj-001", "proj-002"])

            expected_failed_count = 2  # 与 sync_projects_to_worker_with_info 的入参列表长度一致
            assert results["synced"] == []
            assert results["skipped"] == []
            assert len(results["failed"]) == expected_failed_count
    ...
    async def test_sync_project_not_found(self, mock_worker):
        ...
        with (
            ...
            service = WorkerProjectSyncService()
            results, info = await service.sync_projects_to_worker_with_info(mock_worker, ["nonexistent"])
    ...
    async def test_sync_s3_project_with_presigned_url(self, mock_worker, mock_project):
        ...
        with (
            ...
            service = WorkerProjectSyncService()
            results, info = await service.sync_projects_to_worker_with_info(mock_worker, ["proj-001"])
    ...
    @pytest.mark.asyncio
    async def test_sync_fails_when_presigned_url_generation_fails(self, mock_worker, mock_project):
        """测试预签名 URL 生成失败时同步失败"""
        mock_sync_service = MagicMock()
        mock_sync_service.get_project_transfer_info = AsyncMock(
            return_value={
