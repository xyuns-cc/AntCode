...
P0-03 修正后，失败结算必须保留 ownership 并继续续租，否则 PEL 重投
可能被其它 Worker 接管并重复执行业务。只有结果与 ACK 全部成功才 release。

本测试锁死:
1. 上报成功 → release 被调
2. 上报失败(retry 耗尽 raise) → release 不得被调
...
@pytest.mark.asyncio
async def test_release_not_called_on_report_failure():
    """report 失败时保留 ownership，避免另一 Worker 重复执行。"""
    engine, transport = _make_engine()
...
@pytest.mark.asyncio
async def test_release_not_called_on_ack_failure():
    """ACK 失败时保留 ownership，等待同一 Worker 的结算恢复。"""
    engine, transport = _make_engine()
    transport.report_result = AsyncMock(return_value=True)
...
@pytest.mark.asyncio
async def test_report_exception_does_not_attempt_release():
    """结果尚未持久化时不得进入 ownership release。"""
    engine, transport = _make_engine()
    transport.report_result = AsyncMock(return_value=False)
    release = AsyncMock(side_effect=RuntimeError("不应调用"))
    engine._release_run_ownership = release  # type: ignore[method-assign]
