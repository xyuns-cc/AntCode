...
from antcode_worker.domain.enums import RunStatus  # noqa: E402
from antcode_worker.domain.models import (  # noqa: E402
    ArtifactRef,  # noqa: E402
    ExecPlan,
    RuntimeHandle,
)
from antcode_worker.executor.artifacts import ArtifactManager  # noqa: E402
from antcode_worker.executor.base import (  # noqa: E402
    BufferedLogSink,
    CallbackLogSink,
    ExecutorConfig,
    ExecutorStats,
...
from antcode_worker.executor.process import ProcessExecutor  # noqa: E402
...
class TestArtifactManager:
    ...
    @pytest.mark.asyncio
    async def test_store_artifact_persists_to_postgres_blob(self, tmp_path):
...
@pytest.mark.asyncio
async def test_process_executor_fails_when_log_write_fails(tmp_path):
    script = tmp_path / "main.py"
    script.write_text("print('line')\n", encoding="utf-8")
    executor = ProcessExecutor(ExecutorConfig(default_timeout=5))
    runtime_handle = RuntimeHandle(
        path=str(tmp_path),
        runtime_hash="runtime-1",
        python_executable=sys.executable,
    )
    exec_plan = ExecPlan(command=str(script), run_id="run-1", timeout_seconds=5)

    result = await executor.run(exec_plan, runtime_handle, FailingLogSink())

    assert result.status == RunStatus.FAILED
    assert "log transport down" in result.error_message
...
class TestBasicSandbox:
    ...
    def test_rule_plugin_keeps_only_scoped_spider_environment(self):
