...
import pytest
from antcode_core.infrastructure.redis import worker_group
...
class TestTransportConfigValidation:
    ...
    def test_gateway_mode_forbids_localhost_redis_url(self):
        """Gateway 模式禁止配置 localhost redis_url。"""
        config = TransportConfig(
            mode="gateway",
            worker_id="worker-001",
...
class TestBuildTransportConfigFromEnv:
    ...
    def test_build_from_env_direct_mode(self):
        ...
        with patch.dict(
            "os.environ",
            {
                ...
                "WORKER_REDIS_NAMESPACE": "ac-prod",
            ...
            assert config.direct.redis_namespace == "ac-prod"
            assert config.direct.consumer_group == "my-workers"

    def test_build_from_env_uses_namespace_default_consumer_group(self):
        with patch.dict(
            "os.environ",
            {
                "WORKER_TRANSPORT_MODE": "direct",
                "WORKER_ID": "worker-001",
                "WORKER_REDIS_URL": "redis://10.0.0.10:6379/0",
                "WORKER_REDIS_NAMESPACE": "ac-team",
            },
        ):
            config = build_transport_config_from_env()

            assert config.direct.consumer_group == worker_group("ac-team")
    ...
    def test_legacy_worker_env_names_are_ignored(self):
        with patch.dict(
            "os.environ",
            {
                "ANTCODE_WORKER_ID": "legacy-worker",
                "ANTCODE_API_KEY": "legacy-key",
                "REDIS_NAMESPACE": "legacy-namespace",
            },
            clear=True,
        ):
            config = build_transport_config_from_env()
            ...
            assert config.direct.redis_namespace == "antcode"
