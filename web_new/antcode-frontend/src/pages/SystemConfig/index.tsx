    ...
    >
      ...
      >
        <Form.Item
          ...
          rules={[{ required: true, message: '请选择或输入调度器时区' }]}
          tooltip={`${CONFIG_FIELD_DESCRIPTIONS.scheduler_timezone}。⚠️ 此配置修改后需要重启服务才能生效。`}
        >
          <TimezoneSelect placeholder="请选择或输入时区，如 Asia/Shanghai" />
        </Form.Item>
  ...
  if (loading) {
    return (
      <div className={styles.loadingContainer}>
        <Spin size="large" />
        <span style={{ marginLeft: 12, color: 'var(--text-secondary)' }}>加载配置中...</span>
      </div>
    )
  }
