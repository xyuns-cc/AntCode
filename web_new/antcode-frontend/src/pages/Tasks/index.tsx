      ...
      <Card>
        <ResponsiveTable
          ...
          columns={[
            ...
            {
              title: '执行 Worker',
              dataIndex: 'specified_worker_name',
              key: 'specified_worker_name',
              width: 130,
              responsive: ['lg'],
              render: (_: string, record: Task) => {
                const workerName = record.specified_worker_id ? (record.specified_worker_name || record.specified_worker_id) : '自动选择'
                const icon = record.specified_worker_id 
                  ? <CloudServerOutlined style={{ fontSize: 12 }} /> 
                  : <CloudServerOutlined style={{ fontSize: 12 }} />
                return (
                  <Tooltip title={workerName} placement="topLeft">
                    <Tag 
                      color={record.specified_worker_id ? 'cyan' : 'green'}
                      style={{ 
                        maxWidth: '100%', 
                        overflow: 'hidden', 
                        textOverflow: 'ellipsis', 
