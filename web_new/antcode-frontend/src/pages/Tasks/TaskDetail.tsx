...
const TaskDetail: React.FC = () => {
  ...
  const handleCancel = async (executionId: string) => {
    ...
    try {
      ...
      if (result.remote_cancelled) {
        showNotification('success', '任务已取消，已发送取消指令到 Worker')
      } else {
  ...
  // 检查是否为 Worker 不可用错误
  const isWorkerUnavailableError = (errorMessage?: string) => {
    if (!errorMessage) return false
    return errorMessage.includes('No available Worker nodes') ||
           errorMessage.includes('无可用 Worker') ||
           errorMessage.includes('Worker 不可用') ||
           errorMessage.includes('Worker 不存在') ||
           errorMessage.includes('Worker 不在线') ||
           errorMessage.includes('Worker 未连接') ||
           (errorMessage.includes('Worker') && errorMessage.includes('离线')) ||
           errorMessage.includes('未绑定执行 Worker') ||
           errorMessage.includes('未指定执行 Worker') ||
           errorMessage.includes('未启用故障转移')
  }
  ...
  const executionColumns = [
    ...
    },
    {
      ...
      key: 'status',
      width: 90,
      render: (status: TaskStatus, record: TaskExecution) => {
        const config = formatStatus(status)
        // 如果是失败状态且有 Worker 不可用错误，显示特殊标记
        if (status === 'failed' && isWorkerUnavailableError(record.error_message)) {
          return (
            <Tooltip title="Worker 不可用导致失败">
              <Tag color="error" icon={<CloudServerOutlined />}>Worker 不可用</Tag>
            </Tooltip>
          )
        }
    ...
    },
    {
      ...
      width: 200,
      ellipsis: { showTitle: false },
      render: (errorMessage: string | undefined, record: TaskExecution) => {
        if (!errorMessage && record.status !== 'failed') return '-'
        if (!errorMessage) return '-'
        
        // Worker 不可用错误使用特殊样式
        if (isWorkerUnavailableError(errorMessage)) {
          return (
            <Tooltip title={errorMessage} placement="topLeft">
              <Tag color="warning" style={{ maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                <CloudServerOutlined style={{ marginRight: 4 }} />
                {errorMessage.length > 25 ? `${errorMessage.substring(0, 25)}...` : errorMessage}
              </Tag>
            </Tooltip>
          )
        }
        ...
        return (
          <Tooltip title={errorMessage} placement="topLeft">
            <span style={{ color: token.colorError, fontSize: '12px' }}>
    ...
    },
    {
      ...
      render: (_: unknown, record: TaskExecution) => (
        <Space size="small">
          ...
          {(record.status === 'running' || record.status === 'pending' || record.status === 'dispatching' || record.status === 'queued') && (
            <Tooltip title="取消执行">
              <Button
                type="text"
                size="small"
                ...
                <Row gutter={24}>
                  <Col span={16}>
                    <Descriptions title="基本信息" bordered column={2}>
                      ...
                      <Descriptions.Item label="执行 Worker" span={2}>
                        {task.specified_worker_id ? (
                          <Space>
                            <CloudServerOutlined style={{ color: '#1890ff' }} />
                            <span>{task.specified_worker_name || task.specified_worker_id}</span>
                            <Tag color="blue">远程 Worker</Tag>
                          </Space>
                        ) : (
                          <Space>
