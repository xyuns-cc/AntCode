...
const ExecutionLogs: React.FC = () => {
      ...
      execution
        ? [
            ...
            ...(execution.error_message ? [{
              key: 'errorMessage',
              label: '错误信息',
              value: isWorkerUnavailableError(execution.error_message) ? (
  ...
  return (
    <div style={{ 
      ...
      {/* Worker 不可用错误提示 */}
      {execution && execution.status === 'failed' && isWorkerUnavailableError(execution.error_message) && (
        <Alert
          message="任务执行失败：Worker 不可用"
          description={
            <div>
              <p style={{ margin: '0 0 8px 0' }}>{execution.error_message}</p>
              <p style={{ margin: 0, color: token.colorTextSecondary }}>
                请检查 Worker 状态，确保至少有一个在线 Worker 可用于任务执行。
              </p>
            </div>
          }
          type="error"
        ...
        <div style={{
          ...
          <EnhancedLogViewer
            ...
            onStatusUpdate={(statusUpdate) => {
              ...
              if (['SUCCESS', 'FAILED', 'TIMEOUT', 'CANCELLED', 'SKIPPED', 'REJECTED'].includes(upperStatus)) {
                loadExecution()
              }
            }}
          />
