...
import { TASK_EXECUTION_STRATEGY_OPTIONS } from '@/types'
...
const TaskCreate: React.FC = () => {
  ...
  const executionStrategy = Form.useWatch('execution_strategy', form)
  ...
  const loadWorkers = useCallback(async () => {
    try {
      const workerList = await workerService.getMyAvailableWorkers()
  ...
  const handleSubmit = async (values: TaskCreateRequest) => {
    setLoading(true)
    try {
      const selectedStrategy = values.execution_strategy || undefined
      const specifiedWorkerId =
        selectedStrategy === 'specified' ? values.specified_worker_id : undefined
      ...
      const taskData: TaskCreateRequest = {
        ...
        execution_strategy: selectedStrategy,
      ...
      >
        ...
        >
          ...
          <Alert
            ...
            showIcon
            message="任务执行策略"
            description="任务默认继承项目的执行策略配置。如需覆盖，可选择指定 Worker 或自动选择。"
          ...
          <Row gutter={24}>
            <Col span={12}>
              <Form.Item
                ...
                tooltip="默认继承项目配置，可选择自动或指定 Worker"
              >
                <Select
                  placeholder="继承项目配置"
                  allowClear
                >
                  {TASK_EXECUTION_STRATEGY_OPTIONS.map(option => (
                    <Option key={option.value || 'inherit'} value={option.value}>
                      <Space>
                        ...
                        {option.value === '' && <Tag color="default">推荐</Tag>}
                      </Space>
                    </Option>
                  ))}
                </Select>
            ...
            {executionStrategy === 'specified' && (
              <Col span={12}>
                <Form.Item
                  ...
                  rules={[{ required: true, message: '请选择执行 Worker' }]}
                >
                  <Select
